...
from flow_grpo.diffusers_patch.sd3_pipeline_with_logprob import pipeline_with_logprob_hcy
...
def main(_):
    ...
    if config.prompt_fn == "general_ocr":
        ...
        train_sampler = DistributedKRepeatSampler( 
            ...
            k=config.sample.num_image_per_prompt_before_pruning, # 16
            num_replicas=accelerator.num_processes, # 4
        ...
        train_sampler = DistributedKRepeatSampler( 
            ...
            k=config.sample.num_image_per_prompt_before_pruning,
    ...
    train_neg_prompt_embeds = neg_prompt_embed.repeat(config.train.batch_size, 1, 1) # 检查此处
    sample_neg_pooled_prompt_embeds = neg_pooled_prompt_embed.repeat(config.sample.train_batch_size, 1)
    train_neg_pooled_prompt_embeds = neg_pooled_prompt_embed.repeat(config.train.batch_size, 1) # 检查此处

    if config.sample.num_image_per_prompt == 1:
        config.per_prompt_stat_tracking = False
    # initialize stat tracker
    if config.per_prompt_stat_tracking:
        stat_tracker = PerPromptStatTracker(config.sample.global_std) # 可以看出确实和家琪学长讲的一样，这里是global的方差
    ...
    while True:
        ...
        for i in tqdm(
            ...
            rewards = executor.submit(reward_fn, images, prompts, prompt_metadata, only_strict=True) # prompt_metadata在general_ocr里是空的，可以不用修改；也就是说如果后续要使用PickScore外的奖励函数要重新设计prompt_metadata，因为prompts变了
            # yield to to make sure reward computation starts
            time.sleep(0)
        ...
        # num_batches = config.sample.num_batches_per_epoch # 这之后所有的num_batches_per_epoch是否都要变化
